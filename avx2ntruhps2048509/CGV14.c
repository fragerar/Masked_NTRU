...
void convert_A2B_CGV14_cadd_1(Masked* y, const Masked* x, unsigned k){
  /* Convert from A2B and add 1 if the value is > (1<<(k-1))*/
  uint32_t a[N_SHARES], b[N_SHARES], c[N_SHARES], t[N_SHARES] ;
  for(int i=0; i < N_SHARES; ++i) a[i] = (uint32_t) x->shares[i];
  ConvertAB(a, b, k, N_SHARES);
  for(int i=0; i < N_SHARES; ++i) b[i] = b[i]%NTRU_Q;

  for(int i=0; i < N_SHARES; ++i) c[i] = (b[i] >> (NTRU_LOGQ-1))&1;

  SecAdd(b, c, a, k+1, N_SHARES);

  
  for(int i=0; i < N_SHARES; ++i) y->shares[i] = (uint16_t) a[i];
}
